...
This validation case was picked particly because it does share similar with the inlet boundary conditions for a CD nozzle. In a inlet boundary for a CD nozzle requires one of the variables to be float at the inlet to keep the throat chocked (Anderson, 1995). Hence at the inlet, total temperature and total pressure was choosen as the boundary conditions, allowing velocity to float. The total pressure and total temperature value where set using the cases specific ratios, $$\frac{p_t}{p_{ref}} = 1.02828$$ and $$\frac{T_t}{T_{ref}} = 1.008$$, with $$T_{ref} = 300$$ K and $$p_{ref} = 101325$$ Pa. The outlet only has a fixed pressure, again using the ratio specificed by (AIAA TMRWG, 2026), $$\frac{p}{p_{ref}} = 1$$, so p = 101325 Pa, same as the initial internal pressure field, where all other variables float. The top patch boundary condition was set to freestream conditions using the reference velocity, pressure, and temperature values that are also used for the internal field, hence achieve the zero pressure gradient along the y axis. On the bottom surface, between the inlet and the plate, a symmetry condition is used to stop....
